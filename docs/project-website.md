...
- Hosted MCP endpoint via Streamable HTTP/SSE for AI clients.
...
│   │   ├── api/[transport]/  # MCP transport endpoints (/api/mcp, /api/sse)
...
| `/api/sse` | SSE MCP endpoint for resumable streaming clients |
