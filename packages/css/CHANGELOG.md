...
- 563e301: Add `surface-glass-liquid` (white-alpha-30 light, black-alpha-20 dark): the tint for a glass pane the platform already makes legible, such as iOS 26 Liquid Glass. `surface-glass` stays the tint for panes that carry their own contrast (the web's `backdrop-filter`, the SwiftUI material fallback).

## 1.8.0

### Minor Changes

- 89bdba4: Publish `tokens.json`: every token resolved per mode, with each brand's override inline and what was skipped and why — the same resolution the Swift and Kotlin artifacts come from, for consumers that are programs. `cocso/mobile` parsed `CocsoTokens.swift` with regular expressions; it can read this instead.

## 1.7.0

### Minor Changes

- d0ff69b: Add `surface-glass`, `surface-glass-active` and `border-glass`: the tint, pressed tint and edge of a glass surface — a translucent pane over content that scrolls beneath it (tab bars, navigation bars, floating buttons and cards). Light: 80% white (60% pressed) / 10% black edge; dark: 60% black (40% pressed) / 20% white edge. Opaque enough that `text-primary` on the pane clears WCAG AA over any backdrop.

## 1.6.0

### Minor Changes

- aaa4397: `theme-cocso.css` 를 추가했습니다 — cocso.co.kr 의 브랜드 색(info 램프)을 베이스 위에
