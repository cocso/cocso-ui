...
describe('Typography', () => {
  describe('default rendering', () => {
    ...
    it('renders children as h1 when type="display"', () => {
      ...
      expect(screen.getByRole('heading', { level: 1, name: 'Display text' })).toBeInTheDocument();
    });
  ...
  describe('color CSS variable', () => {
    it('sets --cocso-typography-font-color when color is provided', () => {
      const { container } = render(<Typography color="red">Colored</Typography>);
      const el = container.firstChild as HTMLElement;
      expect(el.style.getPropertyValue('--cocso-typography-font-color')).toBe('red');
    });
  ...
  describe('font-weight CSS variable', () => {
    it.each([
      ...
      const { container } = render(<Typography weight={weight}>Text</Typography>);
      const el = container.firstChild as HTMLElement;
      expect(el.style.getPropertyValue('--cocso-typography-font-weight')).toBe(expectedValue);
    });

    it('defaults to bold weight for type="heading"', () => {
      const { container } = render(<Typography type="heading">Heading</Typography>);
      const el = container.firstChild as HTMLElement;
      expect(el.style.getPropertyValue('--cocso-typography-font-weight')).toBe('700');
    });

    it('defaults to normal weight for type="body"', () => {
      const { container } = render(<Typography type="body">Body</Typography>);
      const el = container.firstChild as HTMLElement;
      expect(el.style.getPropertyValue('--cocso-typography-font-weight')).toBe('400');
    });
  });
  ...
  describe('font-size CSS variable', () => {
    ...
    it('sets default font size to 16px for type="custom" without size', () => {
      ...
      const el = container.firstChild as HTMLElement;
      expect(el.style.getPropertyValue('--cocso-typography-font-size')).toBe('16px');
    });
    ...
    it('sets responsive font-size CSS variables for type="display"', () => {
      ...
      expect(el.style.getPropertyValue('--cocso-tablet-typography-font-size')).toBe('60px');
    });
  });
