...
describe('Select', () => {
  ...
  describe('behavior', () => {
    ...
    it('reflects the selected value', async () => {
      ...
      await userEvent.selectOptions(screen.getByRole('combobox'), 'b');
      expect(screen.getByRole('option', { name: 'Option B' })).toHaveProperty('selected', true);
    });
  ...
  describe('size CSS variables', () => {
    it.each([
      ...
      expect(select).toHaveStyle({ '--cocso-select-font-size': expectedFontSize });
    });

    it('applies default size "md" CSS variables when no size prop is given', () => {
      render(
        <Select aria-label="Choose option">
          <option value="a">Option A</option>
        </Select>
