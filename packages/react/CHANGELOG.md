...
- 45be30f: 꺼진 스위치의 손잡이에 테두리가 생깁니다.
...
- ec600a7: `Input`과 `Select`의 쉬는 테두리 값을 레시피로 옮겼습니다.
...
- 6edabbc: `ButtonSize` 타입에 `"x-large"` 가 빠져 있던 것을 고쳤습니다. 1.3.0 은 CSS 와
...
- 92997e4: Button 에 `size="x-large"` (56px) 를 추가했습니다. 화면 하단을 차지하는 전폭 CTA 를
  위한 크기로, `cocso/mobile` 이 손으로 그리던 높이입니다. 세 플랫폼이 같은
  레시피에서 받습니다.

### Patch Changes

- b95a5a6: `Button size="x-large"` 의 모서리 반경이 크기에 따라 16px 로 커집니다 (x-small 6, small–large 8 의 진행을 잇습니다).
...
- cb5f977: 단위를 받지 않는 CSS 속성에 붙던 `px`를 제거했습니다.

  `font-weight: 700px`는 유효하지 않은 선언이라 브라우저가 통째로 버립니다. 생성된
  `typography.css`·`pagination.css`가 그 값을 내보내고 있었고, `spinner.css`는 날
  개수를 `--cocso-spinner-blades: 10px`로 내보내고 있었습니다.

  **정정 (이 항목의 첫 판은 틀렸습니다):** 처음에는 이 때문에 배포된 제목과 활성
  페이지네이션 항목이 굵게 표시되지 않았다고 적었습니다. 사실이 아닙니다.
  `Typography`와 `Pagination`은 생성된 CSS를 import하지 않고 — 굵기는 `weight`
  prop이 인라인 변수로, 페이지네이션은 자체 모듈 CSS가 정합니다 — `Spinner`는
  생성 CSS를 import하지만 날 개수를 TS 지오메트리에서 읽습니다. 잘못된 선언은
  아무 컴포넌트도 소비하지 않는 아티팩트 안에만 있었고, 화면에 보이는 회귀는
  없었습니다. 생성기와 아티팩트가 잘못된 CSS를 내보내던 것을 고친 것이 이 변경의
  전부이며, 그 문장을 검증 없이 적은 것은 제 실수입니다.
...
- d527924: Give `OneTimePasswordField` an accessible name.
...
- ddf2417: Lift `Checkbox` and `Switch` to the minimum pointer target size.
...
- 6540f5f: Honour `prefers-reduced-motion`, and mirror spacing in RTL.
...
- a418350: Give `Progress` an accessible name.
...
- 8a651b6: Add an opt-in password reveal button to `Input`.
