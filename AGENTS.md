...
- Baselines are captured on the CI runner image, so they MUST be regenerated by the `Update Visual Regression Baselines` workflow (`workflow_dispatch`, pick the branch), never committed from a developer machine — font rendering differs per OS.
