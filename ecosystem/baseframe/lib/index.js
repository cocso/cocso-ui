...
}, MergedStream, onMergedStreamFinished = async (passThroughStream, streams) => {
...
}, noop = () => {}, updateMaxListeners = (passThroughStream, increment) => {
...
var isFile, isDirectory, isSymlink, isFileSync, isDirectorySync, isSymlinkSync;
var init_path_type = __esm(() => {
  isFile = isType.bind(undefined, "stat", "isFile");
  isDirectory = isType.bind(undefined, "stat", "isDirectory");
  isSymlink = isType.bind(undefined, "lstat", "isSymbolicLink");
  isFileSync = isTypeSync.bind(undefined, "statSync", "isFile");
  isDirectorySync = isTypeSync.bind(undefined, "statSync", "isDirectory");
...
var import_fast_glob, import_ignore, defaultIgnoredDirectories, ignoreFilesGlobOptions, GITIGNORE_FILES_PATTERN = "**/.gitignore", applyBaseToPattern = (pattern, base) => isNegativePattern(pattern) ? "!" + path.posix.join(base, pattern.slice(1)) : path.posix.join(base, pattern), parseIgnoreFile = (file, cwd) => {
...
var import_fast_glob2, assertPatternsInput = (patterns) => {
  ...
  files,
  extensions
} = {}) => directoryPaths.flatMap((directoryPath) => isDirectorySync(normalizePathForDirectoryGlob(directoryPath, cwd)) ? getDirectoryGlob({ directoryPath, files, extensions }) : directoryPath), toPatternsArray = (patterns) => {
  ...
  return options;
}, normalizeArguments = (function_) => async (patterns, options) => function_(toPatternsArray(patterns), normalizeOptions2(options)), normalizeArgumentsSync = (function_) => (patterns, options) => function_(toPatternsArray(patterns), normalizeOptions2(options)), getIgnoreFilesPatterns = (options) => {
...
}, globby, globbySync, globbyStream, isDynamicPattern, generateGlobTasks, generateGlobTasksSync;
var init_globby = __esm(() => {
  ...
  isDynamicPattern = normalizeArgumentsSync((patterns, options) => patterns.some((pattern) => import_fast_glob2.default.isDynamicPattern(pattern, options)));
...
var isSchema = (obj) => {
  ...
  if ("extends" in schema.data && schema.data.extends) {
    if (!Array.isArray(schema.data.extends)) {
      errors2.push("data.extends는 배열이어야 해요.");
...
// src/core/token-loader.ts
import { readFile } from "fs/promises";
import { join } from "path";
var loadTokenSchema = async (filePath) => {
  try {
...
}, loadTokenSchemas = async (patterns) => {
  try {
    ...
    return results.filter((schema) => schema !== null);
  } catch (error) {
    console.error("파일 검색 중 오류:", error);
    return [];
  }
}, loadCocsoBaseTokens = async () => {
...
var init_token_loader = __esm(() => {
  init_dist();
  init_globby();
});

// src/core/builders/css-builder.ts
var buildCss = (schemas, options = {}) => {
...
`);
}, resolveTokenReference = (value, allSchemas) => {
  ...
  return value;
}, extractTokensFromSchema = (schema, allSchemas) => {
  const result = {};
  ...
  return result;
}, buildCssFile = (schemas, options = {}) => {
  const css = buildCss(schemas, options);
  return css + `
`;
};
var init_css_builder = () => {};

// src/core/builders/json-builder.ts
var buildJson = (schemas, options = {}) => {
  ...
  for (const schema of schemas) {
    ...
    const tokens = extractTokensFromSchema2(schema);
    Object.assign(result, tokens);
  }
  return formatOutput ? JSON.stringify(result, null, 2) : JSON.stringify(result);
}, extractTokensFromSchema2 = (schema) => {
  const result = {};
  if (schema.data?.tokens) {
    for (const [tokenName, tokenDef] of Object.entries(schema.data.tokens)) {
...
var parseCliArgs = (args) => {
  ...
  return formats.filter((format) => validFormats.includes(format));
}, DEFAULT_PATTERNS, DEFAULT_OUTPUT_DIR = "dist", showHelp = () => {
  console.log(`
...
// src/cli/build.ts
var executeBuildCommand = async (args) => {
  ...
  try {
    ...
    if (verbose)
      console.log("\uD83D\uDCE6 COCSO 기본 토큰을 로드합니다...");
...
// src/cli/validate.ts
var executeValidateCommand = async (args) => {
  const inputPattern = args.inputPattern || DEFAULT_PATTERNS.join(",");
...
init_cli();
init_core();
if (__require.main == __require.module || process.argv[1] === new URL(import.meta.url).pathname) {
