...
/**
 * Merge rules with identical selectors into single rule blocks.
 * Preserves property insertion order so later rules override earlier ones.
 */
function mergeRules(rules: CSSRule[]): CSSRule[] {
  const merged = new Map<string, CSSRule>();
  for (const rule of rules) {
    const existing = merged.get(rule.selector);
    if (existing) {
      Object.assign(existing.properties, rule.properties);
    } else {
      merged.set(rule.selector, { ...rule, properties: { ...rule.properties } });
    }
  }
...
function formatCSS(rules: CSSRule[]): string {
  ...
  // Separate and merge normal vs media-query rules
  const normalRules = mergeRules(rules.filter((r) => !r.mediaQuery));
  ...
  for (const [query, groupRules] of mediaGroups) {
    const mergedGroupRules = mergeRules(groupRules);
    lines.push(`@media ${query} {`);
    for (const rule of mergedGroupRules) {
      lines.push(`  ${rule.selector} {`);
      for (const [prop, value] of Object.entries(rule.properties)) {
        lines.push(`    ${prop}: ${value};`);
      }
...
>(recipe: RecipeDefinition<V, S>): string {
  ...
  if (!isSingleSlot) {
    throw new Error(`Multi-slot runtime generation not yet implemented for recipe "${recipe.name}"`);
  }
...
>(recipe: RecipeDefinition<V, S>): string {
  if (recipe.slots.length > 1) {
    throw new Error(`Multi-slot type generation not yet implemented for recipe "${recipe.name}"`);
  }
  const dims = Object.entries(recipe.variants).map(([dim, values]) => ({
    dim,
    values: Object.keys(values as Record<string, unknown>),
